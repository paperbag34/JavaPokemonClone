...
public class Game {
	...
	static ArrayList<Moves> activePokemonMoves = new ArrayList<Moves>();
	static ArrayList<Moves> activeEnemyPokemonMoves = new ArrayList<Moves>();
...
//		// moves for each pokemon
		...
		addBulbasaurMoves(activePokemonMoves, pokemonMoves);
		...
		while (!done) {
			...
			//save new pokemon to party
			for (int x = 0; x < pokemonParty.size(); ++x) {
				GameMechanics.saveToPokemonParty("PokemonParty.json", pokemonParty.get(x), true);
				System.out.println("Saved to party");
			}
			
			//Save pokemons moves to file
			for (int x = 0; x < activePokemonMoves.size(); ++x) {
				//Change the file to be adjustable in the future saving of moves
				GameMechanics.saveToPokemonMoveList("Pokemon0Moves.json", activePokemonMoves.get(x), true);
				System.out.println("Saved to party");
			}
	...
	private static void gatherCombatResponse() {
		...
		try {
			...
			if (input.equals("fight")) {
				...
				for (int x = 0; x < activePokemonMoves.size(); ++x) {
					System.out.print(x + ") ");
					System.out.println(activePokemonMoves.get(x).toStringShort());
				}
